...
*/
class InfoAdmin extends MY_Controller
{
	private $__title    = "Thông tin tài khoản Admin";
	private $__content  = "Tài khoản Admin";
	function __construct()
	{
		$this->_className = trim(strtolower(__CLASS__));
		parent::__construct();
		$this->load->model("infoadmin_model");
		$this->load->library("form_validation");
	}

	public function index($mess = "")
	{
		$this->breadcrumbs->unshift($this->__title,"/infoadmin/index");
		...
		if ($action == "view-info")
		{
			...
			$this->breadcrumbs->push($header['title'], "/infoadmin/index/view-info/" . $id);
		}
		elseif ($action == "edit-info")
		{
			$header['title'] = 'Thay đổi thông tin tài khoản';
			$header['content'] = 'Thay đổi thông tin tài khoản';
			$this->breadcrumbs->push($header['title'], "/infoadmin/index/edit-info/" . $id);
		}
		else
		{
		...
		$data['admin'] = $this->infoadmin_model->get_all_data_admin();
		...
		{
			$this->load->view('infoadmin/index_view', $data);
		}
	...
	public function viewInfo()
	{
		...
		{
			$data = array();
			$data = $this->infoadmin_model->get_data_info_admin_by_id($id);
			$this->load->view("infoadmin/info_view.php", $data);
		}
		else
		{
			show_404();
	...
	public function editInfo()
	{
		...
		if (!empty($id) && is_numeric($id))
		{
			...
			$this->load->view("infoadmin/edit_view.php", $data);
		}
		else
		{
			show_404();
			...
			if ($this->input->post())
			{
				...
				if ($this->form_validation->run())
				{
					...
					if ($flag)
					{
						...
						$editAdmin = $this->infoadmin_model->edit_admin($data, $username);
						($editAdmin) ? redirect(site_url('infoadmin/index?mess=success')) : redirect(site_url('infoadmin/index/edit-info/'.$id ."/?mess=error"));
					}
					else
					{
						$this->session->set_userdata('error', $errorCheck);
						redirect(site_url('infoadmin/index/edit-info/'.$id ."?mess=fail"));
					}
				}
				else
				{
	...
	public function deleteInfo($id = null)
	{
		if (is_numeric($id))
		{
			$deleteData = $this->infoadmin_model->delete_info($id);

			($deleteData == TRUE) ? redirect(site_url('infoadmin/index?mess=delete-ok')) : redirect(site_url('infoadmin/index?mess=delete-fail'));
		}
		else
		{
			show_404();
